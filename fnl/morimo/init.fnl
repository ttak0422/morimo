...
(set vim.g.morimo (vim.tbl_deep_extend :force {:plugins []}
                                       (or vim.g.morimo {})))
...
(lua "package.preload['morimo.colors']=function()return C end")

(local M {})
;; load plugin highlights manually
(tset M :load
      (eval-compiler
        "function(n)for k,v in pairs(require('morimo.plugins.'..n)(C))do vim.g.morimo.plugins[n]=true;vim.api.nvim_set_hl(0,k,v)end end"))
