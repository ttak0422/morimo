...
;; color palette
(local C (include :morimo.colors))

(lua "local function L(n)for k,v in pairs(require('morimo.plugins.'..n)(C))do vim.g.morimo.plugins[n]=true;vim.api.nvim_set_hl(0,k,v)end end")
...
{;; initialize colorscheme (usually called by :colorscheme command)
 :init (fn []
         ...
         (each [k _ (pairs vim.g.morimo.plugins)]
           (L k))
